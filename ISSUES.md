...
| High | **Fixed locally** | The ledger test/API name and bound semantics were inconsistent. | `ledger_v2_test.cpp` uses the inclusive `UnclaimedIntentsThrough` contract, while the header and implementation exposed the exclusive `UnclaimedIntentsBelow` name. | Keep the inclusive API covered by `ledger_v2_test`. |
| High | **Fixed locally** | WAL pruning removed ordinary PUT history in addition to settled transit pairs. | `ledger_v2_test` caught that pruning must retain PUT/DEL/CHECKPOINT records; the focused ledger suite now passes. | Keep pruning limited to settled transit records and run the complete C++ suite. |
