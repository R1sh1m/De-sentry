...
Status VectorHnswLiteBackend::Open(const std::string& data_dir, uint64_t quota_mb,
  ...
  auto store_or = SegmentStore::Open(dir_ + "/vector.dsf", 256, dek_);
...
Status VectorHnswLiteBackend::LoadManifest() {
  auto text_or = at_rest::ReadSealedJsonFile(manifest_path_, dek_, "vector-manifest");
  if (!text_or.ok()) {
    if (text_or.status().code() == StatusCode::kNotFound) return Status::OK();
    return Status::Corruption(std::string("vector manifest: ") + text_or.status().message());
  }
...
Status VectorHnswLiteBackend::SaveManifest() {
  ...
  Status st = at_rest::WriteSealedJsonFile(manifest_path_, JsonValue(std::move(arr)).Dump(),
                                             dek_, "vector-manifest");
  if (!st.ok()) {
    return Status::IOError(std::string("vector: cannot commit ") + manifest_path_ + ": " +
                           st.message());
  }
  return Status::OK();
}
